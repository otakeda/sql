  ...
  CURSOR cUser IS
        SELECT
            ...
            CASE WHEN PO_EMP_DEPT.DEPT_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL THEN '1'
                 ...
                 ELSE '0'
            END UP_EMPDEPT,
            CASE WHEN DEM.SUBSYSTEM_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.DEM_FLG = '1'   THEN '1'
                 WHEN DEM.SUBSYSTEM_CD IS NOT NULL AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.DEM_FLG = '1'   THEN '2'
                 ELSE '0'
            END AS UP_DEM,
            CASE WHEN DEMA.SUBSYSTEM_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.ADMIN_FLG = '1' THEN '1'
                 WHEN DEMA.SUBSYSTEM_CD IS NOT NULL AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.ADMIN_FLG = '1'  THEN '2'
                 ELSE '0'
            END UP_DEMA,
            CASE WHEN KBB.SUBSYSTEM_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.ADMIN_FLG = '1' THEN '1'
                 WHEN KBB.SUBSYSTEM_CD IS NOT NULL AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.PRIMARY_FLG='1' AND HR_USER.ADMIN_FLG = '1'  THEN '2'
                 ELSE '0'
            END UP_KBB,
            CASE WHEN DAF.SUBSYSTEM_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.DAF_FLG = '1' THEN '1'
                 WHEN DAF.SUBSYSTEM_CD IS NOT NULL AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.DAF_FLG = '1' THEN '2'
                 ELSE '0'
            END UP_DAF,
            CASE WHEN DSV.SUBSYSTEM_CD IS NULL     AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.REPORT_FLG = '1' THEN '1'
                 WHEN DSV.SUBSYSTEM_CD IS NOT NULL AND HR_DEPT.DEPT_CD IS NOT NULL AND HR_USER.REPORT_FLG = '1' THEN '2'
                 ELSE '0'
            END UP_DRS,
        ...
        FROM
            ...
            (SELECT
                ...
                EMP.EMP_NO
            FROM
                M_SUBSYSTEM_ACCESS SUBSYS,
                M_EMP EMP
            WHERE
                SUBSYS.CORP_CD=CNST_CORP_CD AND
                SUBSYS.SUBSYSTEM_CD='DEM' AND
                SUBSYS.EMP_CD = EMP.EMP_CD
            ) DEM,
            (SELECT
                SUBSYS.CORP_CD,
                SUBSYS.DEPT_CD,
        ...
        WHERE
            ...
            HR_USER.DEPT_CD = PO_EMP_DEPT.DEPT_CD(+)      AND
            HR_USER.DEPT_CD = DEM.DEPT_CD(+)         AND
            HR_USER.EMP_NO = DEM.EMP_NO(+)       AND
            HR_USER.DEPT_CD = DEMa.DEPT_CD(+)         AND
            HR_USER.EMP_NO = DEMa.EMP_NO(+)       AND
            HR_USER.DEPT_CD = KBB.DEPT_CD(+)         AND
            HR_USER.EMP_NO = KBB.EMP_NO(+)       AND
            HR_USER.DEPT_CD = DAF.DEPT_CD(+)         AND
            HR_USER.EMP_NO = DAF.EMP_NO(+)           AND
            HR_USER.DEPT_CD = DSV.DEPT_CD(+)         AND
            HR_USER.EMP_NO = DSV.EMP_NO(+) 
      ...
      BEGIN
        ...
        ELSIF vUser(i).UP_EMP ='2' THEN
            ...
            set 
              ...
              DEE_USER_ID = TRIM(vUser(i).EMP_NO), 
              DELETE_FLAG = '0', 
        ...
        -- M_SUBSYSTEM_ACCESS(DAF)==============================================
        IF vUser(i).UP_DAF = '1' THEN
          ...
          var_insert_cnt_daf := var_insert_cnt_daf + 1;
        END IF; -- END OF "IF vUser(i).UP_DAF IS NULL THEN"
        ...
        IF vUser(i).UP_KBB = '1' AND CNST_CORP_CD = '21000188' THEN
          insert into PORTAL_USER.M_SUBSYSTEM_ACCESS (
            EMP_CD,
            CORP_CD,
            DEPT_CD,
          ...
          var_insert_cnt_kbb := var_insert_cnt_kbb + 1;
        END IF; -- END OF "IF vUser(i).UP_KBB IS NULL THEN"

        IF vUser(i).UP_DRS = '1' THEN
          insert into PORTAL_USER.M_SUBSYSTEM_ACCESS (
            EMP_CD,
            CORP_CD,
            DEPT_CD,
          ...
          var_insert_cnt_dsv := var_insert_cnt_dsv + 1;
  ...
  WHERE EXISTS(
            select 1 from
                ...
                select
                    ...
                    M_EMP_DEPT.CORP_CD,
                    M_EMP_DEPT.DEPT_CD ,
                    WK_IMPORT_EMP.DEPT_CD as DELETE_FLAG
                from
                    M_EMP_DEPT 
                        left outer join M_EMP on
                            M_EMP_DEPT.EMP_CD = M_EMP.EMP_CD
                        left outer join WK_IMPORT_EMP
                             on
                                M_EMP.EMP_NO = WK_IMPORT_EMP.EMP_NO and
                                M_EMP_DEPT.DEPT_CD = WK_IMPORT_EMP.DEPT_CD
                where
                ...
                from
                    M_SUBSYSTEM_ACCESS 
                        ...
                        left outer join WK_IMPORT_EMP
                             on
                                M_EMP.EMP_NO = WK_IMPORT_EMP.EMP_NO and
                                M_SUBSYSTEM_ACCESS.DEPT_CD = WK_IMPORT_EMP.DEPT_CD
                where
  ...
  select count(EMP_CD) into var_notexist_if_cnt 
               FROM (
                    select
                        ...
                        WK_IMPORT_EMP.DEPT_CD AS IMPORT_DEPT
                    from
                        M_EMP_DEPT 
                        left outer join M_EMP on
                            M_EMP_DEPT.EMP_CD = M_EMP.EMP_CD
