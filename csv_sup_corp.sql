...
--- 20090901 osamu takeda added "update" processing
 	update  m_corp m set update_date = sysdate, (corp_name,tdb_corp_cd,corp_group_cd) =
  	(select t.corp_name,t.tdb_corp_cd,corp_group_cd from wk_import_corp t where m.corp_cd=t.corp_cd)
  	where exists (select 1 from  wk_import_corp t where m.corp_cd=t.corp_cd);
